...
void UJHCombatComponent::ApplyDamage(const FAttackInfo& AttackInfo)
{
    ...
    bool bResult = GetWorld()->SweepSingleByChannel(
        HitResult,
        aLocation,
        aLocation + aForwardVector * AttackInfo.Range,
        FQuat::Identity,
        ECollisionChannel::ECC_GameTraceChannel2,
        FCollisionShape::MakeSphere(AttackInfo.Radius),
        Params
    );
...
void UJHCombatComponent::ApplyAOEDamage(const FAttackInfo& AttackInfo)
{
    FHitResult HitResult;
    FCollisionQueryParams Params(NAME_None, false, GetOwner());
    FVector aLocation = GetOwner()->GetActorLocation();
